...
class COCODataset(CocoDetection):
    ...
    def __getitem__(self, index):
        ...
        ann = [o for o in ann if o['iscrowd'] == 0]  # 不挤的物体
        boxes = [o['bbox'] for o in ann]
        keypoints = [o["keypoints"] for o in ann]
        ...
        img = np.array(img)  # 去掉 pil 对象的一些属性

        img, boxes, keypoints = self.preprocess_img_boxes(img, boxes, self.resize_size, keypoints)
        ...
        return img, boxes, classes, keypoints

    def preprocess_img_boxes(self, image, boxes, input_ksize, keypoints=None):
        """
        ...
        if boxes is None:
            ...
            keypoints[:, 2::3] = marks
            return image_paded, boxes, keypoints
    ...
    def collate_fn(self, data):
        imgs_list, boxes_list, classes_list, keypoints_list = zip(*data)
        ...
        for i in range(batch_size):
            ...
            pad_keypoints_list.append(
                torch.nn.functional.pad(keypoints_list[i], (0, 0, 0, max_num - keypoints_list[i].shape[0]), value=-1))
        batch_boxes = torch.stack(pad_boxes_list)
        batch_classes = torch.stack(pad_classes_list)
        batch_imgs = torch.stack(pad_imgs_list)
        batch_keypoints = torch.stack(pad_keypoints_list)

        return batch_imgs, batch_boxes, batch_classes, batch_keypoints


if __name__ == "__main__":
    dataset = COCODataset("F:\\Object-Detection\\datasets\\coco\\images\\val2017",
                          "F:\\Object-Detection\\datasets\\coco\\annotations\\person_keypoints_val2017.json")
    # instances_val2017.json

    img, boxes, classes, keypoints = dataset[0]
    print(
        boxes, classes, keypoints, "\n",
        img.shape, boxes.shape, classes.shape, keypoints.shape, "\n",
        boxes.dtype, classes.dtype, img.dtype, keypoints.dtype
    )

    img, boxes, classes, keypoints = dataset.collate_fn([dataset[0], dataset[1], dataset[2]])
    print(
        boxes, classes, keypoints, "\n",
        img.shape, boxes.shape, classes.shape, keypoints.shape, "\n",
