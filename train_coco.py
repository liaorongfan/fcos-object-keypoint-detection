...
train_dataset = COCODataset(
    "/data/Datasets/COCO/train2017",
    "/data/Datasets/COCO/annotations/person_keypoints_train2017.json",
    transform=transform
)
...
for epoch in range(EPOCHS):
    for epoch_step, data in enumerate(train_loader):
        ...
        batch_imgs, batch_boxes, batch_classes, keypoints = data
        ...
        optimizer.zero_grad()
        losses = model([batch_imgs, batch_boxes, batch_classes, keypoints])
        loss = losses[-1]
        loss.mean().backward()
        ...
        print(
            "global_steps:%d epoch:%d steps:%d/%d cls_loss:%.4f cnt_loss:%.4f reg_loss:%.4f kpt_loss:%.4f cost_time:%dms lr=%.4e "
            "total_loss:%.4f" % \
            (GLOBAL_STEPS, epoch + 1, epoch_step + 1, steps_per_epoch, losses[0].mean(), losses[1].mean(),
             losses[2].mean(), losses[3].mean(), cost_time, lr, loss.mean())
        )

        GLOBAL_STEPS += 1
    torch.save(model.state_dict(), "./checkpoint02_kptbranch_fulltrain/model_{}.pth".format(epoch + 1))
    print("model saved")
