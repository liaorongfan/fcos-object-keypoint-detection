...
class FCOS(nn.Module):
    ...
    def forward(self, x):
        ...
        cls_logits, cnt_logits, reg_preds, keypoint_pred = self.head(all_P)
        return [cls_logits, cnt_logits, reg_preds, keypoint_pred]
...
class DetectHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        key_preds, _ = self._reshape_cat_out(inputs[3], self.strides)  # [bs, sum(_h * _w), 34
        ...
        keypoints = self._coords2keypoints(coords, key_preds)
        # select topk
        max_num = min(self.max_detection_boxes_num, cls_scores.shape[-1])  # 300
        ...
        for batch in range(cls_scores.shape[0]):
            ...
            _keypoints.append(keypoints[batch][topk_ind[batch]])
        ...
        keypoints_topk = torch.stack(_keypoints, dim=0)  # [batch_size,max_num,4]
        assert boxes_topk.shape[-1] == 4
        return self._post_process([cls_scores_topk, cls_classes_topk, boxes_topk, keypoints_topk])
    ...
    def _post_process(self, preds_topk):
        ...
        _boxes_post = []
        _keypoints_post = []
        cls_scores_topk, cls_classes_topk, boxes_topk, keypoints_topk = preds_topk
        for batch in range(cls_classes_topk.shape[0]):
            ...
            _keypoints_post_b = keypoints_topk[batch][mask]
            ...
            _keypoints_post.append(_keypoints_post_b[nms_ind])
        # scores, classes, boxes = torch.stack(_cls_scores_post, dim=0), torch.stack(_cls_classes_post,
        #                                                                            dim=0), torch.stack(_boxes_post,
        #                                                                                                dim=0)
        post_processed = [
            torch.stack(processed, dim=0) for processed in [
                _cls_scores_post, _cls_classes_post, _boxes_post, _keypoints_post
            ]
        ]
        return post_processed  # scores, classes, boxes, {}
    ...
    def box_nms(boxes, scores, thr):
        ...
        while order.numel() > 0:
            if order.numel() == 1:
                i = order.item()  # convert one element tensors to python scalars
                keep.append(i)
                break
            else:
                i = order[0].item()
    ...
    @staticmethod
    def _coords2keypoints(coords, offsets):
        key_points_coords_all = torch.cat([coords for _ in range(17)], dim=-1)
        key_points_all = key_points_coords_all[None, :, :] - offsets
        return key_points_all
    ...
    def _reshape_cat_out(self, inputs, strides):
        ...
        c = inputs[0].shape[1]  # channel numbers
        out = []
        coords = []
        for pred, stride in zip(inputs, strides):
            pred = pred.permute(0, 2, 3, 1)  # (bs, h, w, c)
...
class FCOSDetector(nn.Module):
    def __init__(self, mode="training", config=None):
        ...
        if config is None:
            config = DefaultConfig
        self.mode = mode
        self.fcos_model = FCOS(config=config)
        if mode == "training":
    ...
    def forward(self, inputs):
        ...
        if self.mode == "training":
            batch_imgs, batch_boxes, batch_classes, batch_keypoints = inputs
            # 模型输出
            out = self.fcos_model(batch_imgs)
            # 编码标签信息
            targets = self.target_layer([out, batch_boxes, batch_classes, batch_keypoints])
            # 计算标签和预测信息间的损失
            ...
            batch_imgs = inputs
            out = self.fcos_model(batch_imgs)
            scores, classes, boxes, keypoints = self.detection_head(out)
            boxes = self.clip_boxes(batch_imgs, boxes)
            return scores, classes, boxes, keypoints
