...
class ClsCntRegHead(nn.Module):
    def __init__(self, in_channel, class_num, GN=True, cnt_on_reg=True, prior=0.01):
        ...
        for i in range(4):
            ...
            kpt_branch.append(nn.Conv2d(in_channel, in_channel, kernel_size=3, padding=1, bias=True))
            if GN:
                kpt_branch.append(nn.GroupNorm(32, in_channel))
            kpt_branch.append(nn.ReLU(True))


        self.cls_conv = nn.Sequential(*cls_branch)
        self.reg_conv = nn.Sequential(*reg_branch)
        self.kpt_conv = nn.Sequential(*kpt_branch)
        ...
        self.keypoints_pred = nn.Conv2d(in_channel, 34, kernel_size=3, padding=1)
    ...
    def forward(self, inputs):
        ...
        for index, P in enumerate(inputs):
            ...
            keypoints_pred.append((self.keypoints_pred(kpt_conv_out)))
            # keypoints_pred.append(self.scale_exp[index](self.keypoints_pred(reg_conv_out)))
            # keypoints_pred.append(self.keypoints_pred(reg_conv_out))
        return cls_logits, cnt_logits, reg_preds, keypoints_pred
