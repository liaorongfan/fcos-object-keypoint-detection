...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("./checkpoint005_kptbranch/model_24.pth", map_location=torch.device('cpu')))
    ...
    root = "/data/Datasets/voc/VOCdevkit/VOC2012/JPEGImages/"
    ...
    for name in names:
        ...
        # print(out)
        scores, classes, boxes, keypoints = out
        boxes = boxes[0].cpu().numpy().tolist()
        keypoints = keypoints[0].cpu().numpy().tolist()
        keypoints = [[item[i: i + 2] for i in range(0, 34, 2)] for item in keypoints]
        ...
        for i, box in enumerate(boxes):
            keypoints_box = keypoints[i]
            pt1 = (int(box[0]), int(box[1]))
            pt2 = (int(box[2]), int(box[3]))
            img_pad = cv2.rectangle(img_pad, pt1, pt2, (0, 255, 0))
            for point in keypoints_box:
                x, y = int(point[0]), int(point[1])
                # img_pad = cv2.circle(img_pad, (x, y), 20, (255, 255, 0))
                ax.plot(x, y, 'o')
            ...
            ax.add_patch(bbox)
            plt.text(
                box[0], box[1], s="%s %.3f" % (COCODataset.CLASSES_NAME[int(classes[i])], scores[i]),
                color='white',
